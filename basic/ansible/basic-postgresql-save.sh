...
echo "[RUN] ansible-playbook -v -i "$INVENTORY" --extra-vars "version=$XYZLivraison" --extra-vars "@variables/variables-$ENV.yml" --extra-vars "@../ansible/nexus.yml" $VERSION/postgresql-save.yml"

rm $VERSION/postgresql-save.retry
ansible-playbook -v -i "$INVENTORY" --extra-vars "version=$XYZLivraison" --extra-vars "@variables/variables-$ENV.yml" --extra-vars "@../ansible/nexus.yml" $VERSION/postgresql-save.yml
