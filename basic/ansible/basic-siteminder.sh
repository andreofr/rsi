...
echo "[RUN] ansible-playbook -v -i "$INVENTORY" --extra-vars "version=$XYZLivraison" --extra-vars "@variables/variables-$ENV.yml" --extra-vars "@../ansible/nexus.yml" $VERSION/siteminder.yml"

rm $VERSION/siteminder.retry
ansible-playbook -v -i "$INVENTORY" --extra-vars "version=$XYZLivraison" --extra-vars "@variables/variables-$ENV.yml" --extra-vars "@../ansible/nexus.yml" $VERSION/siteminder.yml
